    ...
    if (!marker_image.empty())
        {
            ...
            cv::aruco::estimatePoseSingleMarkers(corners, 0.10, camera_matrix, dist_coeffs,
                                                 rvecs, tvecs); //0.086 marker 大小
            ...
            {
                cv::aruco::drawAxis(marker_image, camera_matrix, dist_coeffs, rvecs,
                                    tvecs, 0.1); //画坐标轴
                Zc = tvecs[0][2] - 0.044;
                cout << "深度输出:" << Zc << endl;
